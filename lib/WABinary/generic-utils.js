...
// gua coba fix error rate-limit
const assertNodeErrorFree = async (node) => {
    ...
    if (errNode) {
        ...
        if (errorCode === 429) { // rate limit
            let retries = 3; // coba 3x sebelum nyerah
            while (retries > 0) {
                console.log(`kena rate limit, nunggu 5 detik (percobaan ke-${4 - retries})...`);
                await delay(5000);
                retries--;
                return; // keluar dari function biar gak ngelempar error
            }
        }
